...
const double encoder_per_rotation_m1 =  116.0494;                                              // encoder value per rotation of motor1 [from 65535 - 65305]
const double encoder_per_rotation_m2 = 123.8;     //125.6296;                                              // encoder value per rotation of motor2 [from 0 - 28911]
...
void EncoderCallback(const geometry_msgs::Vector3::ConstPtr& encoder_ticks)
{
    ...
    {
        dm1 = (encoder_ticks->x > _x_tick) ? (encoder_ticks->x - 65535 -_x_tick) * distance_per_count_m1 : (encoder_ticks->x + (65535 - _x_tick)) * distance_per_count_m1;
    }
    else
    {
    ...
    {
        dm2 = (encoder_ticks->y > _y_tick) ? (encoder_ticks->y - 65535 - _y_tick) * distance_per_count_m2 : (encoder_ticks->y + (65535 - _y_tick)) * distance_per_count_m2;
    }
    else
    {
